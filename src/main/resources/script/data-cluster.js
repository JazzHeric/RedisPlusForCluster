...
//重载库
function loadDbTree() {
    var json = dataClusterRouter.likeInit(currNode0.pattern);
    var data = JSON.parse(json);
    currNode0.name = data.data.name;
    $.fn.zTree.getZTreeObj("keyTree").updateNode(currNode0);
}
...
function loadDbData(node, pattern) {
    layer.load(2);
    var json = dataClusterRouter.treeData(node.id, node.page, pattern);
    var data = JSON.parse(json);
    layer.closeAll('loading');
    if (data.code === 200) {
...
function getKeysInfo(order) {
    ...
    layer.load(2);
    var json = dataClusterRouter.keysData(currNode1.name, order);
    var data = JSON.parse(json);
    layer.closeAll('loading');
    if (data.code === 200) {
...
function updateStr() {
    ...
    layer.load(2);
    var json = dataClusterRouter.updateStr(currNode1.name, str);
    var data = JSON.parse(json);
...
function insertSet() {
    ...
    layer.prompt({
        ...
        layer.load(2);
        var json = dataClusterRouter.insertVal(1, currNode1.name, text, 0);
        var data = JSON.parse(json);
        layer.closeAll('loading');
        layer.close(index);
        if (data.code === 200) {
...
function updateSet(val) {
    ...
    layer.prompt({
        ...
        var json = dataClusterRouter.updateVal(JSON.stringify(param));
        var data = JSON.parse(json);
        layer.closeAll('loading');
        layer.close(index);
        if (data.code === 200) {
...
function insertZset() {
    ...
    layer.prompt({
        ...
        yes: function (index) {
            ...
            layer.load(2);
            var json = dataClusterRouter.insertVal(2, currNode1.name, key, sco);
            var data = JSON.parse(json);
            layer.closeAll('loading');
            layer.close(index);
            if (data.code === 200) {
...
function insertList() {
    ...
    layer.prompt({
        ...
        layer.load(2);
        var json = dataClusterRouter.insertVal(3, currNode1.name, text, 0);
        var data = JSON.parse(json);
        layer.closeAll('loading');
        layer.close(index);
        if (data.code === 200) {
...
function insertHash() {
    layer.prompt({
        ...
        yes: function (index) {
            ...
            var json = dataClusterRouter.insertVal(4, currNode1.name, mapVal, 0);
            var data = JSON.parse(json);
            layer.closeAll('loading');
            layer.close(index);
            if (data.code === 200) {
